...
# plot.wd		<- getwd()
out.file	<- "cvs_baseline_matchagg_ht"
ww			<- 6.5
ar			<- .6
...
table(mypan$ban_ard, mypan$frac_seg)
...
rr  	<- Match(Y=match.shtdat[,"trip_cvs"], Tr=Tr, X=pshat, M=my.ratio, 
...
est.ols		<- est.mat	<- est.iv	<- est.fe	<- matrix(NA, length(dv.col)*2, 4, 
														dimnames = list(paste(rep(dv.col, each = 2), c("Treated", "Treated*Heavy"), sep="-"), c("Estimate", "Std. Error", "t value", "Pr(>|t|)")))											
for(i in 1:length(dv.col)){
	selidx		<- 	1:2 + (i-1)*2
	dv.fml		<- as.formula(paste(dv.col[i], "~ treat + treat*heavy"))

	# OLS
	est.ols[selidx,]	<- coeftest(lm(dv.fml, data = match.shtdat))[c("treat","treat:heavy"),]	

	# Matching
	est.mat[selidx,]	<- coeftest(lm(dv.fml, data = match.shtdat[sel,]))[c("treat","treat:heavy"),]
	
	# Distance as IV
	est.iv[selidx,]	<- coeftest(ivreg(dv.fml, instruments = ~ heavy+log(distance_cvs) + log(distance_cvs)*heavy, 
					data = match.shtdat))[c("treat","treat:heavy"),]
	
	# Fixed effect model with matched households
	tmp		<- plm(as.formula(paste(dv.col[i], "~ treat + treat*after + after + heavy + after*heavy + treat*heavy*after")), data = tmpdat, 
					index = c("household_code", "month"), model = "within")
	cls.v	<- Cls.se.fn(tmp, cluster.vec = tmpdat[,"household_code"], return.se = FALSE)
	est.fe[selidx,]	<- coeftest(tmp, vcov = cls.v)[c("treat:after", "treat:after:heavy"),]
}
...
# ---------------- #
# Add covariates #
est1.ols		<- est1.mat	<- est1.iv	<- est1.fe	<- matrix(NA, length(dv.col)*2, 4, 
														dimnames = list(paste(rep(dv.col, each = 2), c("Treated", "Treated*Heavy"), sep="-"), c("Estimate", "Std. Error", "t value", "Pr(>|t|)")))
sel			<- match.shtdat$household_code %in% mypan[unlist(rr[c("index.treated", "index.control")]), "household_code"]
table(mypan[sel,"frac_seg"])
...
for(i in 1:length(dv.col)){
	...
	# OLS
	est1.ols[selidx,]	<- coeftest(lm(dv.fml, data = match.shtdat))[c("treat","treat:heavy"),]

	# Matching without replacement 
	est1.mat[selidx,]	<- coeftest(lm(dv.fml, data = match.shtdat[sel,]))[c("treat","treat:heavy"),]

	# Distance as IV
	est1.iv[selidx,]	<- coeftest(ivreg(dv.fml, instruments = ~ income + age + have_kids + employment + race + distance_cvs + 
											    pre_q + pre_trip_total + pre_trip_othchannel + pre_dol_total + 
											    pre_dol_othchannel + log(distance_cvs) + heavy + heavy:log(distance_cvs), 
							data = match.shtdat))[c("treat","treat:heavy"),]

	# Fixed effect model with matched households
	tmp		<- plm(update(dv.fml, ~ . + treat*after + after + heavy*after + treat*after*heavy), data = tmpdat, 
					index = c("household_code", "month"), model = "within")
	cls.v	<- Cls.se.fn(tmp, cluster.vec = tmpdat[,"household_code"], return.se = FALSE)
	est1.fe[selidx,]	<- coeftest(tmp, vcov = cls.v)[c("treat:after","treat:heavy:after"),]
}
...
stargazer(list(est.ols, est.mat, est.iv, est.fe, est1.ols, est1.mat, est1.iv, est1.fe), 
		  type = "html", align = TRUE, no.space = TRUE, digits = 2,
		  column.labels = c("OLS", "Matching", "IV", "Panel", "OLS.ctr", "Matching.ctr", "IV.ctr", "Panel.ctr"), 
