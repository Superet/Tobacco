...
for(i in 1:length(dv.col)){
	...
	est.s2[selidx,]	<- coeftest(lm(dv.fml, data = match.shtdat[sel,]))[c("treat","s2"),]
	est.largecvs[selidx,]	<- coeftest(lm(update(dv.fml, .~.-s2-treat*s2+large_cvs + treat), data = match.shtdat[sel,]))[c("treat","large_cvs"),]
	est.heavy[selidx,]	<- coeftest(lm(update(dv.fml, .~.-s2-treat*s2+heavy+treat*heavy), data = match.shtdat[sel,]))[c("treat","heavy:treat"),]
	est.wgr[selidx,]	<- coeftest(lm(update(dv.fml, .~.-s2-treat*s2+cls_wgr+treat*cls_wgr), data = match.shtdat[sel,]))[c("treat","cls_wgr:treat"),]
}

# Print results 
sel 	<- seq(2, nrow(est.s2), 2)
tmp1	<- list(est.s2[-sel,], est.s2[sel,])
tmp1	<- lapply(tmp1, function(x) {rownames(x) <- dv.col; class(x) <- "coeftest"; x})
...
tmp2	<- lapply(tmp2, function(x) {rownames(x) <- dv.col; class(x) <- "coeftest"; x})
...
tmp3	<- lapply(tmp3, function(x) {rownames(x) <- dv.col; class(x) <- "coeftest"; x})
...
tmp4	<- lapply(tmp4, function(x) {rownames(x) <- dv.col; class(x) <- "coeftest"; x})
