...
plot.wd		<- getwd()
out.file	<- "cvs_nonsmk_match_small"
ww			<- 6.5
ar			<- .6
...
cat(sum(mypan$frac_seg == "Zero"),"households out of ", nrow(mypan), "shopped at CVS but did not buy cigarettes there, so drop them for this current analysis.\n")
mypan		<- subset(mypan, frac_seg != "Never")
mypan		<- subset(mypan, !(frac_seg == "Zero" & smk == 1))
table(mypan$smk, mypan$frac_seg)
