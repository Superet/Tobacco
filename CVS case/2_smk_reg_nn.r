...
match.mod	<- matchit(fml, method = "nearest", data = mypan[,c("household_code","treat",match.col)], 
						ratio = my.ratio, replace = FALSE, caliper = my.caliper)
...
nn		<- blc$nn
sel 	<- c("Means Treated", "Means Control", "Mean Diff", "SD Control")
tmp		<- blc$sum.matched[,sel]
names(tmp)	<- paste("M_", names(tmp), sep="")
tmp		<- cbind(blc$sum.all[,sel], tmp)
tmp1	<- c(blc$nn[1,], NA, NA, blc$nn[2,], NA, NA)					# Number of controls and treatment
blc		<- rbind(tmp1, tmp)
...
sel		<- match.pan$treat == 1
pdf(paste("~/Desktop/fg_", out.file,"_qq.pdf",sep = ""), width = 10, height = 10)
par(mfrow = c(3,3))
for(i in selcol){
	rg	<- range(match.pan[,i])
	qqplot(match.pan[sel,i], mypan[!sel,i], xlim = rg, ylim = rg,xlab = "Treated", ylab = "Control", main = i) 
}
dev.off()
...
sel			<- match.shtdat$household_code %in% match.pan$household_code
table(mypan[sel,"frac_seg"])
tmpdat		<- subset(smk.trips, household_code %in% match.pan$household_code &
