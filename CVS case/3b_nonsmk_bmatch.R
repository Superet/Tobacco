...
tmp2<- sapply(mb$AfterMatching, function(x) c(x$mean.Co,x$mean.Tr, x$sdiff.pooled/100, x$tt$p.value, 
										ifelse("ks" %in% names(x), x$ks$ks.boot.pvalue, x$tt$p.value) ))
tmp3<- MatchBalance(fml, data=subset(mypan, household_code %in% c(nonsmk.match)), nboots=numbt)															
...
nn		<- setNames(c(length(unique(rr$index.control)), length(unique(rr$index.treated))), c("Control", "Treated"))												
cat("The number of unique households:\n"); print(nn); cat("\n")
cat("Balance check:\n"); print(round(blc,2)); cat("\n")

stargazer(blc[,-grep("Std", colnames(blc))], summary = FALSE, type = "latex", label = "tab:blc_nonsmk", 
			title = "Balance Check Between Non-smokers and Treated Smokers", 
			digits = 2)
...
# Check bipart matching 
sel	<- which(!nonsmk.match %in% mypan$household_code, arr.ind = TRUE)
if(length(sel) > 0){
	sel	<- sel %% nrow(nonsmk.match)
	nonsmk.match 	<- nonsmk.match[-sel,]
}
...
sel.bp		<- match.shtdat$household_code %in% c(nonsmk.match)
...
class(est.bipart)	<- class(est.ols)	<- class(est.mat) <- class(est.fe) <- "coeftest"
...
class(est1.bipart)	<- class(est1.ols)	<- class(est1.mat) <- class(est1.fe) <- "coeftest"
...
stargazer(list(OLS = est1.ols, Matching = est1.mat, Bipart = est1.bipart, est1.fe), type = "latex", 
			no.space = FALSE, align = FALSE, digits = 2, 
			title = "Regression with non-smokers wtih covariates using non-smoker data",
			column.labels = c("OLS", "Matching", "Bipartite", "Panel"),
			covariate.labels = dv.lab,
...
# 			title = "Balance Check among smokers", notes = paste(names(nn), " (", nn, ")", sep="", collapse = ","), 
...
stargazer(list(est.ols, est.mat, est.fe, est.bipart,est1.ols, est1.mat, est1.fe, est1.bipart), 
		  ...
		  column.labels = rep(c("OLS", "Matching", "Panel", "Bipartite"), 2),
		  covariate.labels = dv.lab,
