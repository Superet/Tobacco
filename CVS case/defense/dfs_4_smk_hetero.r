...
grp.lab	<- c("Cigarette-driven CVS shoppers", "Non cigarette-driven CVS shoppers", 
			 ...
			 "Large share of cigarette wallet at CVS", "Small share of cigarette wallet at CVS")
tmp				<- matrix(NA, length(selcol), 3*4, dimnames = list(selcol, c(rbind(matrix(grp.lab, 2,4), ""))))
sel1			<- list(mypan$s2 		== 1, 
						mypan$heavy 	== 1, 
						mypan$cls_wgr	== 1, 
						mypan$large_cvs	== 1)
for(i in selcol){
...
dim(match.shtdat)
match.shtdat	<- merge(match.shtdat, mypan[,c("household_code", "treat", "frac_seg", "s2", "large_cvs","heavy", "cls_wgr",match.col)], 
...
est.largecvs	<- est.s2	<- est.heavy	<- est.wgr	<- matrix(NA, length(dv.col)*2, 4, 
												dimnames = list(NULL, c("Estimate", "Std. Error", "t value", "Pr(>|t|)")))
cov.largecvs	<- cov.s2	<- cov.heavy	<- cov.wgr	<- matrix(NA, length(dv.col), 2, dimnames = list(dv.col, c(0, 1)))

sel			<- match.shtdat$household_code %in% mypan[unlist(rr[c("index.treated", "index.control")]), "household_code"]
# sel		<- match.shtdat$household_code %in% c(match.pan)


for(i in 1:length(dv.col)){
	selidx			<- 1:2 + (i-1)*2
	(dv.fml			<- as.formula(paste(dv.col[i], "~ treat + s2 + treat*s2 +", paste(match.col, collapse = "+"))))
	
	tmp					<- lm(dv.fml, data = match.shtdat[sel,])
	v					<- vcov(tmp)[c("treat","s2"),c("treat","s2")]
	est.s2[selidx,]		<- coeftest(tmp)[c("treat","s2"),]
	cov.s2[i,]			<- c(v[1], sum(v))
	
	tmp					<- lm(update(dv.fml, .~.-s2-treat*s2+large_cvs + treat), data = match.shtdat[sel,])
	v					<- vcov(tmp)[c("treat","large_cvs"),c("treat","large_cvs")]
	est.largecvs[selidx,]<- coeftest(tmp)[c("treat","large_cvs"),]
	cov.largecvs[i,]	<- c(v[1], sum(v))
	
	tmp					<- lm(update(dv.fml, .~.-s2-treat*s2+heavy+treat*heavy), data = match.shtdat[sel,])
	v					<- vcov(tmp)[c("treat","heavy:treat"),c("treat","heavy:treat")]
	est.heavy[selidx,]	<- coeftest(tmp)[c("treat","heavy:treat"),]
	cov.heavy[i,]		<- c(v[1], sum(v))
	
	tmp					<- lm(update(dv.fml, .~.-s2-treat*s2+cls_wgr+treat*cls_wgr), data = match.shtdat[sel,])
...
# Print results 
sel 	<- seq(2, nrow(est.s2), 2)
tmp1	<- list(est.s2[-sel,], est.s2[sel,])
tmp1	<- lapply(tmp1, function(x) {rownames(x) <- dv.col; class(x) <- c("coeftest", "matrix"); x})
names(tmp1)	<- c("Treat", "Treat*CigarDriven")
stargazer(tmp1, type = "text", column.labels = c("Treat", "Treat*CigarDriven"))

tmp2	<- list(est.heavy[-sel,], est.heavy[sel,])
tmp2	<- lapply(tmp2, function(x) {rownames(x) <- dv.col; class(x) <- c("coeftest", "matrix"); x})
names(tmp2)	<- c("Treat", "Treat*Heavy")
stargazer(tmp2, type = "text", column.labels = c("Treat", "Treat*Heavy"))
...
tmp4	<- list(est.largecvs[-sel,], est.largecvs[sel,])
tmp4	<- lapply(tmp4, function(x) {rownames(x) <- dv.col; class(x) <- c("coeftest", "matrix"); x})
names(tmp4)	<- c("Treat", "Treat*LargeCVS_Share")
stargazer(tmp4, type = "text", column.labels = c("Treat", "Treat*LargeCVS_Share"))


# stargazer(list(tmp1, tmp4, tmp3, tmp2), type = "latex", digits = 2,	label = "tab:hetero",
# 		title = "Heterogeneous Treatment Effects", 
# 		covariate.labels = dv.lab,
# 		column.labels = c(names(tmp1), names(tmp4),names(tmp3), names(tmp2)) )

# stargazer(list(tmp1, tmp4, tmp3, tmp2), type = "html", 
...
tmp1	<- c(est.s2[,"Estimate"], est.heavy[,"Estimate"], est.wgr[,"Estimate"], est.largecvs[,"Estimate"])
sel		<- seq(2, length(tmp1), 2)
tmp1[sel]	<- tmp1[-sel] + tmp1[sel]
tmp2	<- c(t(rbind(cov.s2, cov.heavy, cov.wgr, cov.largecvs)))
sel		<- sapply(1:4, function(x) rep(c(2*x, 2*x-1), length(dv.lab)))
ggtmp	<- data.frame(	Model 	= rep(c("Cigar-driven", "Consumption", "Walgreens", "CVS share"), each = nrow(est.s2)), 
						Group 	= grp.lab[c(sel)], 
						DV 		= rep(rep(dv.lab, each = 2), 4), 
...
sel.mod	<- c("Cigar-driven", "CVS share", "Walgreens", "Consumption")
