...
plot.wd		<- getwd()
out.file	<- "cvs_nonsmk_match_heterogeneity"
ww			<- 6.5
ar			<- .6
...
tmp <- data.table(subset(trips, household_code %in% match.pan$household_code & month >= event.month - 3 & month <= event.month + 2))
tmp	<- tmp[,list(total_spent = sum(total_spent)), by = list(treat, household_code, month, purchase_date, channel_type, retailer_code, cvs)]
tmp$after <-  1*(tmp$month >= event.month)
tmp	<- tmp[,list(trip_cvs 		= length(purchase_date[cvs==1]), 
                   trip_othdrug 	= length(purchase_date[channel_type == "Drug Store" & cvs ==0]), 
                   trip_othchannel = length(purchase_date[channel_type != "Drug Store"]), 
                   dol_cvs 		= sum(total_spent*cvs, na.rm = T), 
                   dol_othdrug		= sum(total_spent*(1-cvs)*1*(channel_type == "Drug Store"), na.rm = T), 
...
lm.s	<- list(NULL)
lm.s[[1]]	<- plm(trip_cvs ~ after + frac_seg + after*frac_seg , data = mydata[sel.s,], 
                 index = c("household_code", "month"), model = "within")
lm.s[[2]]	<- plm(trip_othdrug ~ after + frac_seg + after*frac_seg , data = mydata[sel.s,], 
                 index = c("household_code", "month"), model = "within")
lm.s[[3]]	<- plm(trip_othchannel ~ after + frac_seg + after*frac_seg , data = mydata[sel.s,], 
                 index = c("household_code", "month"), model = "within")
lm.s[[4]]	<- plm(dol_cvs ~ after + frac_seg + after*frac_seg , data = mydata[sel.s,], 
                 index = c("household_code", "month"), model = "within") 
lm.s[[5]]	<- plm(dol_othdrug ~ after + frac_seg + after*frac_seg , data = mydata[sel.s,], 
                 index = c("household_code", "month"), model = "within")
lm.s[[6]]	<- plm(dol_othchannel ~ after + frac_seg + after*frac_seg , data = mydata[sel.s,], 
                 index = c("household_code", "month"), model = "within")
lm.s[[7]]	<- plm(dol_total ~ after + frac_seg + after*frac_seg , data = mydata[sel.s,], 
                  index = c("household_code", "month"), model = "within")					
lm.s[[8]]	<- plm(netdol_cvs ~ after + frac_seg + after*frac_seg , data = mydata[sel.s,], 
                  index = c("household_code", "month"), model = "within") 
lm.s[[9]]	<- plm(netdol_othdrug ~ after + frac_seg + after*frac_seg , data = mydata[sel.s,], 
                  index = c("household_code", "month"), model = "within")
lm.s[[10]]	<- plm(netdol_othchannel ~ after + frac_seg + after*frac_seg , data = mydata[sel.s,], 
                  index = c("household_code", "month"), model = "within")									 
...
lm.l	<- list(NULL)
lm.l[[1]]	<- plm(trip_cvs ~ after + frac_seg + after*frac_seg+month1, data = mydata, 
                 index = c("household_code", "month"), model = "within")
lm.l[[2]]	<- plm(trip_othdrug ~ after + frac_seg + after*frac_seg+month1, data = mydata, 
                 index = c("household_code", "month"), model = "within")
lm.l[[3]]	<- plm(trip_othchannel ~ after + frac_seg + after*frac_seg+month1, data = mydata, 
                 index = c("household_code", "month"), model = "within")
lm.l[[4]]	<- plm(dol_cvs ~ after + frac_seg + after*frac_seg+month1, data = mydata, 
                 index = c("household_code", "month"), model = "within") 
lm.l[[5]]	<- plm(dol_othdrug ~ after + frac_seg + after*frac_seg+month1, data = mydata, 
                 index = c("household_code", "month"), model = "within")
lm.l[[6]]	<- plm(dol_othchannel ~ after + frac_seg + after*frac_seg+month1, data = mydata, 
                 index = c("household_code", "month"), model = "within")
lm.l[[7]]	<- plm(dol_total ~ after + frac_seg + after*frac_seg+month1, data = mydata, 
                  index = c("household_code", "month"), model = "within")					
lm.l[[8]]	<- plm(netdol_cvs ~ after + frac_seg + after*frac_seg+month1, data = mydata, 
                  index = c("household_code", "month"), model = "within") 
lm.l[[9]]	<- plm(netdol_othdrug ~ after + frac_seg + after*frac_seg+month1, data = mydata, 
                  index = c("household_code", "month"), model = "within")
lm.l[[10]]	<- plm(netdol_othchannel ~ after + frac_seg + after*frac_seg+month1, data = mydata, 
                  index = c("household_code", "month"), model = "within")									 
...
ivreg.s	<- list(NULL)
ivreg.s[[1]]	<- plm(trip_cvs ~ after + frac_seg + after*frac_seg , data = mydata.match[sel.ivs,], 
                    index = c("household_code", "month"), model = "within")
ivreg.s[[2]]	<- plm(trip_othdrug ~ after + frac_seg + after*frac_seg , data = mydata.match[sel.ivs,], 
                    index = c("household_code", "month"), model = "within")
ivreg.s[[3]]	<- plm(trip_othchannel ~ after + frac_seg + after*frac_seg , data = mydata.match[sel.ivs,], 
                    index = c("household_code", "month"), model = "within")
ivreg.s[[4]]	<- plm(dol_cvs ~ after + frac_seg + after*frac_seg , data = mydata.match[sel.ivs,], 
                    index = c("household_code", "month"), model = "within") 
ivreg.s[[5]]	<- plm(dol_othdrug ~ after + frac_seg + after*frac_seg , data = mydata.match[sel.ivs,], 
                    index = c("household_code", "month"), model = "within")
ivreg.s[[6]]	<- plm(dol_othchannel ~ after + frac_seg + after*frac_seg , data = mydata.match[sel.ivs,], 
                    index = c("household_code", "month"), model = "within")
ivreg.s[[7]]	<- plm(dol_total ~ after + frac_seg + after*frac_seg , data = mydata.match[sel.ivs,], 
                     index = c("household_code", "month"), model = "within")					
ivreg.s[[8]]	<- plm(netdol_cvs ~ after + frac_seg + after*frac_seg , data = mydata.match[sel.ivs,], 
                     index = c("household_code", "month"), model = "within") 
ivreg.s[[9]]	<- plm(netdol_othdrug ~ after + frac_seg + after*frac_seg , data = mydata.match[sel.ivs,], 
                     index = c("household_code", "month"), model = "within")
ivreg.s[[10]]	<- plm(netdol_othchannel ~ after + frac_seg + after*frac_seg , data = mydata.match[sel.ivs,], 
                     index = c("household_code", "month"), model = "within")									 
...
ivreg.l	<- list(NULL)
ivreg.l[[1]]	<- plm(trip_cvs ~ after + frac_seg + after*frac_seg+month1, data = mydata.match, 
                    index = c("household_code", "month"), model = "within")
ivreg.l[[2]]	<- plm(trip_othdrug ~ after + frac_seg + after*frac_seg+month1, data = mydata.match, 
                    index = c("household_code", "month"), model = "within")
ivreg.l[[3]]	<- plm(trip_othchannel ~ after + frac_seg + after*frac_seg+month1, data = mydata.match, 
                    index = c("household_code", "month"), model = "within")
ivreg.l[[4]]	<- plm(dol_cvs ~ after + frac_seg + after*frac_seg+month1, data = mydata.match, 
                    index = c("household_code", "month"), model = "within") 
ivreg.l[[5]]	<- plm(dol_othdrug ~ after + frac_seg + after*frac_seg+month1, data = mydata.match, 
                    index = c("household_code", "month"), model = "within")
ivreg.l[[6]]	<- plm(dol_othchannel ~ after + frac_seg + after*frac_seg+month1, data = mydata.match, 
                    index = c("household_code", "month"), model = "within")
ivreg.l[[7]]	<- plm(dol_total ~ after + frac_seg + after*frac_seg+month1, data = mydata.match, 
                     index = c("household_code", "month"), model = "within")					
ivreg.l[[8]]	<- plm(netdol_cvs ~ after + frac_seg + after*frac_seg+month1, data = mydata.match, 
                     index = c("household_code", "month"), model = "within") 
ivreg.l[[9]]	<- plm(netdol_othdrug ~ after + frac_seg + after*frac_seg+month1, data = mydata.match, 
                     index = c("household_code", "month"), model = "within")
ivreg.l[[10]]	<- plm(netdol_othchannel ~ after + frac_seg + after*frac_seg+month1, data = mydata.match, 
                     index = c("household_code", "month"), model = "within")									 
...
sel <- c("after:frac_segS1", "after:frac_segS2")
...
rownames(tmp1) <- paste(rep(sapply(lm.s, function(x) as.character(x$formula[[2]])), each = length(sel)), sel, sep="**")
class(tmp1)<- "coeftest"
coefls[[1]]<- tmp1

tmp <- lapply(1:length(lm.l), function(i) coeftest(lm.l[[i]], vcov = cls.se.l[[i]])[sel,])
tmp1 <- do.call(rbind, tmp)
rownames(tmp1) <- paste(rep(sapply(lm.l, function(x) as.character(x$formula[[2]])), each = length(sel)), sel, sep="**")
class(tmp1)<- "coeftest"
coefls[[2]]<- tmp1

tmp <- lapply(1:length(ivreg.s), function(i) coeftest(ivreg.s[[i]], vcov = ivcls.se.s[[i]])[sel,])
tmp1 <- do.call(rbind, tmp)
rownames(tmp1) <- paste(rep(sapply(ivreg.s, function(x) as.character(x$formula[[2]])), each = length(sel)), sel, sep="**")
class(tmp1)<- "coeftest"
coefls[[3]]<- tmp1

tmp <- lapply(1:length(ivreg.l), function(i) coeftest(ivreg.l[[i]], vcov = ivcls.se.l[[i]])[sel,])
tmp1 <- do.call(rbind, tmp)
rownames(tmp1) <- paste(rep(sapply(ivreg.l, function(x) as.character(x$formula[[2]])), each = length(sel)), sel, sep="**")
class(tmp1)<- "coeftest"
coefls[[4]]<- tmp1
