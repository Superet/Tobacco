...
trips		<- merge(trips, mypan[,c("household_code","cvs_in2","heavy","ban_ard","frac_seg")], by = "household_code", all.x = T)
...
mydata$treat_seg	<- factor(with(mydata, ifelse(smk == 0 | ban_ard == 1, "Control", as.character(frac_seg))), levels = c("Control", levels(mypan$frac_seg)))
mypan$treat_seg		<- factor(with(mypan, ifelse(smk == 0 | ban_ard == 1, "Control", as.character(frac_seg))), levels = c("Control", levels(mypan$frac_seg)))
trips$treat_seg		<- factor(with(trips, ifelse(smk == 0 | ban_ard == 1, "Control", as.character(frac_seg))), levels = c("Control", levels(mypan$frac_seg))) 
table(mydata$treat)
table(mypan$smk == 1 & mypan$ban_ard == 0)

# Check if any missing values 
sapply(demo.col, function(i) sum(is.na(mydata[,i])))
sapply(demo.col, function(i) sum(is.na(mypan[,i])))

# A subset: drop the smokers who never shop at CVS for cigarettes
sel			<- with(mypan, smk==1 & frac_seg %in% c("Never", "Zero"))
mypan.sub	<- mypan[!sel,]
mydata.sub	<- subset(mydata, household_code %in% mypan.sub$household_code)
trips.sub	<- subset(trips, household_code %in% mypan.sub$household_code)
...
match.mod	<- matchit(treat ~ income + age + have_kids + employment + race + distance_cvs +
				...
				method = "nearest", data = mypan[,c("household_code","treat","treat_seg",demo.col,bhv.col)], ratio = my.ratio)
summary(match.mod)
match.pan	<- match.data(match.mod)
dim(match.pan)
table(match.pan$treat)
table(match.pan$treat_seg)
...
ggtmp	<- ggtmp[,list(total_spent = sum(total_spent)), by = list(treat,treat_seg, household_code, month, week, purchase_date, channel_type, retailer_code, cvs)]
ggtmp	<- ggtmp[,list(	Total = length(purchase_date)/length(unique(household_code)), 
						CVS = length(purchase_date[cvs==1])/length(unique(household_code)), 
						OtherDrug = length(purchase_date[cvs==0 & channel_type == "Drug Store"])/length(unique(household_code)),
						OtherChannel = length(purchase_date[cvs==0 & channel_type != "Drug Store"])/length(unique(household_code)) 
						), 
				by = list(treat_seg, week)]
ggtmp	<- melt(ggtmp, id.vars = c("treat_seg", "week"))		
# ggtmp$treat	<- factor(ggtmp$treat, levels = c(1,0), labels = c("Smoker", "Non-smoker"))
p1 		<- ggplot(ggtmp, aes(week, value, color = treat_seg)) + geom_line() + 
			geom_vline(xintercept = as.numeric(event.date), col = "red") + 
			facet_wrap(~variable, scales = "free") + 
			scale_colour_manual(values = c("grey50", "black", "blue", "green", "yellow")) + 
			theme(legend.position = "bottom") + 
			labs(y = "Trips per person", title = "Shopping trips by consumer groups")
...
ggtmp	<- melt(ggtmp, id.vars = c("treat_seg", "week"))		
# ggtmp$treat	<- factor(ggtmp$treat, levels = c(1,0), labels = c("Smoker", "Non-smoker"))
p2		<- ggplot(ggtmp, aes(week, value, color = treat_seg)) + geom_line() + 
			geom_vline(xintercept = as.numeric(event.date), col = "red") + 
			facet_wrap(~variable, scales = "free") + 
			scale_colour_manual(values = c("grey50", "black", "blue", "green", "yellow")) + 
			theme(legend.position = "bottom") + 
