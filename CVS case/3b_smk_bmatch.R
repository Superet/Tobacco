...
tmp2<- sapply(mb$AfterMatching, function(x) c(x$mean.Co,x$mean.Tr, x$sdiff.pooled/100, x$tt$p.value, 
										ifelse("ks" %in% names(x), x$ks$ks.boot.pvalue, x$tt$p.value) ))
tmp3<- MatchBalance(fml, data=subset(mypan, household_code %in% c(match.pan)), nboots=numbt)															
...
stargazer(blc[,-grep("Std", colnames(blc))], summary = FALSE, type = "latex", label = "tab:blc_smk", 
...
sel.bp		<- match.shtdat$household_code %in% c(match.pan)
...
class(est.bipart)	<- class(est.ols)	<- class(est.mat) <- class(est.iv) <- class(est.fe) <- "coeftest"
...
stargazer(list(est.ols, est.mat, est.bipart), 
		  ...
		  column.labels = rep(c("OLS", "Matching", "Bipartite"), 2),
...
class(est1.bipart)	<- class(est1.ols)	<- class(est1.mat) <- class(est1.iv) <- class(est1.fe) <- "coeftest"
...
nn1		<- rbind(table(mypan$treat), table(mypan[sel,"treat"]), table(mypan$treat), table(mypan[sel,"treat"]), table(mypan[sel.bp,"treat"]))
stargazer(list(est.ols, est.mat, est.iv, est.fe, est.bipart, est1.ols, est1.mat, est1.iv, est1.fe, est1.bipart), 
		  type = "html", summary = FALSE, align = FALSE, no.space = FALSE, digits = 2,
		  column.labels = rep(c("OLS", "Matching", "IV", "Panel", "Bipartite"), 2),
          title = c("Balance Check among smokers", "Before-after difference between treatment and control group for the matched sample during 201406 - 201411"),
		  add.lines = list(c("Control", rep(nn1[,"0"], 2)),c("Treated", rep(nn1[,"1"], 2)), 
							c("Covariates", rep("No", 5), rep("Yes", 5))), 
