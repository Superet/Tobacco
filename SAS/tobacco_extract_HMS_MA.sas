	...
	%do i=2008 %to 2013;
		...
		* Sebsetting 1: restrict to the households who live in the focal counties; 
		...
		proc sql noprint;
			...
			select *
			from panelists&i 
			where fips_state_descr = &sel_state; 
		...
		* Subsetting 2: only the purchase made on the category; 
			...
			select household_code, purchase_date, upc, upc_ver_uc, upcv, quantity as units, total_price_paid, 
					retailer_code, total_spent, A.trip_code_uc
...
proc sort data = purchases; 
	by household_code purchase_date trip_code_uc upcv; 
run;

data tmp;
	set purchases; 
	by household_code purchase_date trip_code_uc upcv; 
	if first.upcv and last.upcv then unique_flag = 1;
	else unique_flag = 0; 
run; 

proc freq data = tmp; table unique_flag; run;

* Simply keep the first observation if any duplicated; 
data purchases(drop = unique_flag); 
	set purchases;
	where unique_flag = 1; 
run;
...
proc sql noprint;
	create table tmp as 
	select household_code, count(unique(purchase_date)) as n
	from purchases
...
save(prod, panelists, purchases, file = "cigar_HMS_MA.rdata")
